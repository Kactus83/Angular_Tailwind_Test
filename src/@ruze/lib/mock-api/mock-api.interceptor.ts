...
import { RUZE_MOCK_API_DEFAULT_DELAY } from './mock-api.constants';
import { RuzeMockApiService } from './mock-api.service';
import { delay, Observable, of, switchMap, throwError } from 'rxjs';

export const mockApiInterceptor = (request: HttpRequest<unknown>, next: HttpHandlerFn): Observable<HttpEvent<unknown>> =>
{
    const defaultDelay = inject(RUZE_MOCK_API_DEFAULT_DELAY);
    const ruzeMockApiService = inject(RuzeMockApiService);

    // Try to get the request handler
    const {
        handler,
        urlParams,
    } = ruzeMockApiService.findHandler(request.method.toUpperCase(), request.url);
